...
/// Output-style name used for the `claw-janitor` persona.
...
pub fn load_janitor_system_prompt_with_context(
    cwd: impl Into<PathBuf>,
    current_date: impl Into<String>,
    os_name: impl Into<String>,
    os_version: impl Into<String>,
    ...
    let sections = SystemPromptBuilder::new()
        .with_os(os_name, os_version)
        .with_model_family(model_family)
        .with_output_style(JANITOR_STYLE_NAME, JANITOR_STYLE_PROMPT)
        .with_project_context(project_context.clone())
        .with_runtime_config(config)
        .build();
    Ok((sections, project_context))
...
/// Convenience wrapper returning only the rendered Janitor prompt sections.
pub fn load_janitor_system_prompt(
    cwd: impl Into<PathBuf>,
    current_date: impl Into<String>,
    os_name: impl Into<String>,
    os_version: impl Into<String>,
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Jclaw settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
