...
fn version_emits_json_when_requested() {
    ...
    assert!(
        parsed["human_readable"]
            .as_str()
            .is_some_and(|text| text.contains("Jclaw")),
...
fn run_claw(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    ...
    for key in [
        ...
        "CLAW_LOG",
        "RUST_LOG",
        // Model env vars leak from the parent shell (e.g. a Claude Code
        // session sets ANTHROPIC_MODEL) and make model validation reject
        // commands before they reach the path under test. Tests that need a
        // specific model set it explicitly via `envs`, which skips removal.
        "CLAW_MODEL",
        "ANTHROPIC_MODEL",
        "ANTHROPIC_DEFAULT_MODEL",
    ] {
